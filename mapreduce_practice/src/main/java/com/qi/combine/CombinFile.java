...
public class CombinFile {
	...
	public static void main(String[] args) {
		...
		try {
			...
			FileInputFormat.addInputPath(job, new Path("/input/TortoiseSVN License.txt"));
...
//			String name = path.substring(25);
		...
		protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, Text>.Context context)
			...
			for (String string : line) {
				if (!("".equals(string))) {
				context.write(outputKey, new Text(string));
				}
			}
	...
	public static class CombinFileReducer extends Reducer<Text, Text, Text, Text> {
		...
		protected void reduce(Text key, Iterable<Text> values,
			...
			for (Text value : values) {
					contex.write(key, new Text(value));
			}
